...
const fetchPrescriptionHistories = () => {
    ...
    layoutStore.isLoading = true;
    apiRequest
        .get(`/api/v1/patient/medical/history/prescription`, {
            params: {
                family_member_id:
                    selectedFamilyMemberId.value === 0
                        ? ""
...
const fetchLabResults = () => {
    ...
    layoutStore.isLoading = true;
    apiRequest
        .get(`/api/v1/patient/medical/history/labresult`, {
            params: {
                family_member_id:
                    selectedFamilyMemberId.value === 0
                        ? ""
...
const fetchRadResults = () => {
    ...
    layoutStore.isLoading = true;
    apiRequest
        .get(`/api/v1/patient/medical/history/radresult`, {
            params: {
                family_member_id:
                    selectedFamilyMemberId.value === 0
                        ? ""
...
const fetchEncounterHistories = () => {
    ...
    layoutStore.isLoading = true;
    apiRequest
        .get(`/api/v1/patient/medical/history/encounters`, {
            params: {
                family_member_id:
                    selectedFamilyMemberId.value === 0
                        ? ""
...
const filterByFamily = () => {
    const selectedFamilyMember = families.value.find(
        (family) => family.id === selectedFamilyMemberId.value
    );
    medicalHistoriesStore.updateSelectedPatient({
...
const initialize = function () {
    if (
        import.meta.env.VITE_IS_LAB_RESULT_BRIDGING === "true" ||
        import.meta.env.VITE_IS_LAB_RESULT_BRIDGING === "TRUE"
    ) {
...
<template>
    ...
    >
        <button
            id="btn-akan-datang"
            @click="modalState.familyMemberFilterModal.show()"
    ...
    <div class="px-4 pt-7">
        ...
        />
        <div
            class="tab-riwayat-medis nav nav-pills d-flex flex-nowrap col-gap-20 mt-4"
        >
            <button
                class="nav-link unit-vital"
                :class="selectedTab === 'unit-vital' ? 'active' : ''"
                data-bs-toggle="pill"
                data-bs-target="#unit-vital"
                role="tab"
                aria-controls="unit-vital"
                aria-selected="true"
                @click="onTabChange('unit-vital')"
            >
                <div class="icon">
            ...
            <button
                class="nav-link resep-obat"
                :class="selectedTab === 'resep-obat' ? 'active' : ''"
                data-bs-toggle="pill"
                data-bs-target="#resep-obat"
                role="tab"
                aria-controls="resep-obat"
                aria-selected="true"
                @click="onTabChange('resep-obat')"
            >
                <div class="icon">
            ...
            <button
                class="nav-link hasil-lab"
                :class="selectedTab === 'hasil-lab' ? 'active' : ''"
                data-bs-toggle="pill"
                data-bs-target="#hasil-lab"
                role="tab"
                aria-controls="hasil-lab"
                aria-selected="true"
                @click="onTabChange('hasil-lab')"
            >
                <div class="icon">
                    <img
                        :src="MicroscopeWhite"
            ...
            <button
                class="nav-link hasil-rad"
                :class="selectedTab === 'hasil-rad' ? 'active' : ''"
                data-bs-toggle="pill"
                data-bs-target="#hasil-rad"
                role="tab"
                aria-controls="hasil-rad"
                aria-selected="true"
                @click="onTabChange('hasil-rad')"
            >
                <div class="icon">
                    <img
                        :src="MicroscopeWhite"
            ...
            <button
                class="nav-link pertemuan"
                :class="selectedTab === 'pertemuan' ? 'active' : ''"
                data-bs-toggle="pill"
                data-bs-target="#pertemuan"
                role="tab"
                aria-controls="pertemuan"
                aria-selected="true"
                @click="onTabChange('pertemuan')"
                form="#"
            >
                <div class="icon">
        ...
        <div class="tab-content mt-4" id="tab-content">
            <section
                class="tab-pane fade"
                :class="selectedTab === 'unit-vital' ? 'show active' : ''"
                id="unit-vital"
                role="tabpanel"
                aria-labelledby="unit-vital"
                tabindex="0"
            >
                <div
                    id="multiselect"
                    class="dropdown filter-sticky d-flex col-gap-20 align-items-center"
                >
                    <p>Tipe</p>
                    <select
                        class="form-select"
                        aria-label="Tipe"
                        v-model="selectedVitalSignType"
                    >
                        <!-- <option value="" selected>{{ $t('history.choose_vital_unit') }}</option> -->
                        <option value="BP1" selected>
                            {{ $t("history.vital_unit.bp1") }}
                        </option>
                        <option value="BP2">
                            {{ $t("history.vital_unit.bp2") }}
                        </option>
                        <option value="TEMP">
                            {{ $t("history.vital_unit.temp") }}
                        </option>
                        <option value="RESP">
                            {{ $t("history.vital_unit.resp") }}
                        </option>
                    </select>
                </div>
                <div
                    v-if="vitalSignHistories.length > 0"
                    class="d-flex flex-column rows-gap-16 mt-4"
                    v-for="history in vitalSignHistories"
                >
                    <VitalSignCard
                        :dateCreated="history.recordDate_yMdHms"
                        :timeCreated="history.recordTime"
                        :registrationNo="history.registrationNo"
                        :vitalSignUnit="history.vitalSignUnit"
                ...
                <div
                    class="text-center"
                    v-if="
                        vitalSignHistories.length < 1 && !layoutStore.isLoading
                    "
                >
                    <img
                        :src="NotFoundImage"
                        alt="Ilustrasi Tidak Ada Data"
                        width="238"
                        height="198"
                        class="d-inline-block"
                    />
                    <p
                        class="mt-3 fs-3 fw-bold"
                        v-html="$t('history.no_vital_unit_result')"
                    ></p>
                </div>

                <div
                    class="d-flex flex-column rows-gap-16 mt-6 px-4"
                    v-if="
                        !layoutStore.isLoading &&
                        vitalSignHistories.length >= 10
                    "
                    @click="loadMore"
                >
            ...
            <section
                class="tab-pane fade"
                :class="selectedTab === 'resep-obat' ? 'show active' : ''"
                id="resep-obat"
                role="tabpanel"
                aria-labelledby="resep-obat"
                tabindex="0"
            >
                <div
                    class="d-flex flex-column rows-gap-16 mt-4"
                    v-for="history in prescriptionHistories"
                >
                    <PrescriptionCard
                        :prescription-no="history.PrescriptionNo"
                        :prescription-date_y-md-hms="
                            history.prescriptionDate_yMdHms
                        "
                ...
                <div
                    ...
                    v-if="
                        prescriptionHistories.length < 1 &&
                        !layoutStore.isLoading
                    "
                >
                    <img
            ...
            <section
                class="tab-pane fade"
                :class="selectedTab === 'hasil-lab' ? 'show active' : ''"
                id="hasil-lab"
                role="tabpanel"
                aria-labelledby="hasil-lab"
                tabindex="0"
            >
                <div
                    class="d-flex flex-column rows-gap-16 mt-4"
                    v-for="result in labResultHistories"
                >
                    <LabResultCard
                        paramedic-name="-"
                ...
                <div
                    class="text-center mt-3"
                    v-if="
                        labResultHistories.length < 1 && !layoutStore.isLoading
                    "
                >
                    <img
                        :src="LabImage"
                    ...
                    />
                    <p class="mt-4 fs-3 fw-bold">
                        {{ $t("history.no_lab_result") }}
                    </p>
                </div>
                <div
                    class="d-flex flex-column rows-gap-16 mt-4 text-center"
                    v-if="$t('history.callcenter')"
                >
                    <p>
                        Apabila membutuhkan informasi lebih lanjut hubungi call
                        center RS :
                    </p>
            ...
            <section
                class="tab-pane fade"
                :class="selectedTab === 'hasil-rad' ? 'show active' : ''"
                id="hasil-rad"
                role="tabpanel"
                aria-labelledby="hasil-rad"
                tabindex="0"
            >
                <div
                    class="d-flex flex-column rows-gap-16 mt-4"
                    v-for="result in radResultHistories"
                >
                    <RadResultCard
                ...
                <div
                    class="text-center mt-3"
                    v-if="
                        radResultHistories.length < 1 && !layoutStore.isLoading
                    "
                >
                    <img
                        :src="LabImage"
            ...
            <section
                class="tab-pane fade"
                :class="selectedTab === 'pertemuan' ? 'show active' : ''"
                id="pertemuan"
                role="tabpanel"
                aria-labelledby="pertemuan"
                tabindex="0"
            >
                <div
                    class="d-flex flex-column rows-gap-16 mt-4"
                    v-for="result in encounterHistories"
                >
                    <EncounterCard
                        :registration-no="result.registrationNo"
                ...
                <div
                    class="text-center mt-3"
                    v-if="
                        encounterHistories.length < 1 && !layoutStore.isLoading
                    "
                >
                    <img
                        :src="Doctor2Image"
                ...
                <div class="modal-body">
                    <form action="" class="d-flex flex-column rows-gap-16">
                        <div>
                            ...
                            >
                                <option
                                    v-for="(member, index) in families"
                                    :value="member.id"
                                >
                                    {{ member.name }}
                                </option>
                        ...
                        <div class="d-flex col-gap-20">
                            <button
                                type="reset"
                                @click="resetFamilyFilter"
                                class="w-50 btn btn-red-500-rounded"
                                data-bs-dismiss="modal"
                            >
                                {{ $t("history.modal_filter.reset") }}
                            </button>
                            <button
                                type="button"
                                ...
                                :class="
                                    selectedFamilyMemberId === 0
                                        ? 'disabled'
                                        : ''
                                "
                                data-bs-dismiss="modal"
                            >
                                {{ $t("history.modal_filter.apply") }}
                            </button>
                        </div>
                    </form>
                </div>
