...
export default {
    ...
    setup() {
        ...
        const getInpatientRooms = () => {
            ...
            apiRequest
                .get(`/api/v1/doctor/inpatient/rooms`)
                .then((response) => {
                    ...
                    inpatientRooms.value.map((inpatientRoom) => {
                        prevData.value.push(inpatientRoom.RoomID);
                    });
                })
                .catch((error) => {
        ...
        const filterInpatientList = () => {
            ...
            apiRequest
                .get(
                    `/api/v1/doctor/inpatient?room_id=${selectedRoomID.value}&prev_data=${prevData.value}`
                )
                .then((response) => {
                    patients.value = [];
...
<template>
    ...
    <section
        class="filter-inpatient filter-sticky-2 p-4 mt-6 bg-white position-sticky"
    >
        <div>
            <p class="w-100 p-2">
                <span v-if="patientCount !== 0"
                    >{{ patientCount }} {{ $t("inpatient.patient_data") }}</span
                >
            </p>
            <div id="multiselect" class="w-100">
                ...
                >
                    <option value="" selected>
                        {{ $t("inpatient.all_room") }}
                    </option>
                    <option
                        v-for="(ir, index) in inpatientRooms"
                        :value="ir.RoomID"
                    >
                        {{ ir.RoomName }}
                    </option>
    ...
    <div
        ...
        v-if="!isLoading && patients.length >= 10"
        @click="loadMore"
    >
        <button type="button" class="btn btn-default">
            {{ $t("inpatient.load_more") }}
        </button>
    </div>
</template>
